...
namespace Challenges.Tests.LeetCode
{
    public class StraightLineTests
    {
        ...
        public void ValidCheckStraightLine_LeetCodeBest(string numbersStr, bool expectedResult)
        {
            ...
            var result = Library.LeetCode.StraightLine.CheckStraightLine_LeetCodeBest(points);
            Assert.True(result == expectedResult, $"{nameof(Library.LeetCode.StraightLine.CheckStraightLine_LeetCodeBest)} does not meet the expected. Solution result is '{result}' but expected is '{expectedResult}' for: '{numbersStr}'");
        }
