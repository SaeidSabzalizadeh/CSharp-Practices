...
namespace Challenges.Tests.LeetCode.June
{
    public class DeleteNodeinLinkedListTests
    {
        ...
        public void ValidDeleteNodeinLinkedList(string inputStr, int deletedItem, string expectedResultStr)
        {
            ...
            Library.LeetCode.June.DeleteNodeinLinkedList.DeleteNode(targetNode);
            Assert.True(inputListNode?.ToString() == expectedResult?.ToString(), $"{nameof(Library.LeetCode.June.DeleteNodeinLinkedList.DeleteNode)} does not meet the expected. Solution result is '{inputListNode}' but expected is '{expectedResultStr}' for: '{inputStr}, node={deletedItem}'");
        }
