...
namespace Challenges.Tests.LeetCode
{
    public class SingleElementInArrayTests
    {
        ...
        public void ValidSingleElementInArray(string numbersStr, int expectedResult)
        {
            ...
            var result = Library.LeetCode.SingleElementInArray.SingleNonDuplicate(nums);
            Assert.True(result == expectedResult, $"{nameof(Library.LeetCode.SingleElementInArray.SingleNonDuplicate)} does not meet the expected. Solution result is '{result}' but expected is '{expectedResult}' for: '{numbersStr}'");
        }
    }
}
